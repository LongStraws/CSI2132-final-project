...
const HotelPage = () => {
    ...
    function onSubmit(data: z.infer<typeof FormSchema>) {
      ...
      if (!isNaN(numericId)) {
        ...
        const start_date = startDate ? format(startDate, "yyyy-MM-dd") : undefined;
    ...
    const handleBookClick = (roomId: number) => {
      if (!availabilityChecked) {
        setDateValidationError('Please check room availability for your selected dates before booking.');
        return;
      }
      if (startDate && endDate) {
        router.push(`/book/${roomId}?start_date=${format(startDate, 'yyyy-MM-dd')}&end_date=${format(endDate, 'yyyy-MM-dd')}`);
      }
    };
    ...
    const fetchHotelRooms = async (numericId: number, start_date?: string, end_date?: string) => {
...
<Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 px-5">
          ...
          <Button type="submit" className="mx-auto">Check Availability</Button>
        </form>
        {dateValidationError && (
        ...
        <TableBody>
            {hotelRooms.map((room) => (
                <TableRow key={room.hotelid}>
                    ...
                    <TableCell>
                      <Button onClick={() => handleBookClick(room.roomid)}>Book</Button>
                    </TableCell>
                </TableRow>
            ))}
        </TableBody>
